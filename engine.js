...
class SpriteActor extends Actor {
    ...
    get width(){
        return Math.ceil(this.imageWidth * this.scale);
    }

    get height(){
        return Math.ceil(this.width * this.aspectRatio);
    }
    ...
    constructor(x, y, sprite, hitArea, tags=[]) {
        ...
        this.sprite = sprite;
        this.imageWidth = sprite.rectangle.width;
        this.imageHeight = sprite.rectangle.height;
    ...
    render(target) {
        ...
        ctx.save();
        ctx.scale(this.hMirror ? -1 : 1, this.vMirror ? -1 : 1);
        ctx.translate(this.hMirror ? -this.width : 0, this.vMirror ? -this.height : 0);
